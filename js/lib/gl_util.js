...
class camera
{
    ...
    // Matrix Outputs:
    proj_mtx       = mat4.create();                        // The projection matrix associated with the camera.
    view_mtx       = mat4.create();                        // The view matrix associated with the camera.

    // Intermediate Data:
    fwd_vec       = vec3.create();                         // An intermediate calculation for traverse_arcball().
    right_vec     = vec3.create();                         // An intermediate calculation for traverse_arcball().
    rot_combined  = quat.create();                         // An intermediate calculation for traverse_arcball().
    rot_horiz     = quat.create();                         // An intermediate calculation for traverse_arcball().
    rot_vert      = quat.create();                         // An intermediate calculation for traverse_arcball().
    ...
    compute_proj_mtx()
    {
        mat4.perspective(this.proj_mtx, this.fov, this.aspect, this.z_near, this.z_far);
        return this.proj_mtx;
    }

    compute_view_mtx()
    {
        mat4.lookAt(this.view_mtx, this.eye_pos, this.targ_pos, this.up_vec);
    ...
    set_eye_pos(x, y, z)
    {
        ...
        this.eye_radius = vec3.distance(this.eye_pos, this.targ_pos);   // "eye_radius" is needed in traverse_arcball().
    }
    ...
    {
        // 1. Calculate the forward and right vectors. "up_vec" is already normalized by set_up_vec() or step 5.
        vec3.subtract(this.fwd_vec, this.targ_pos, this.eye_pos);   // target position - eye position
        vec3.normalize(this.fwd_vec, this.fwd_vec);   // normalized so it can be re-scaled in step 6. Otherwise,
                                                      // "eye_pos" drifts, causing the camera to move.
        vec3.cross(this.right_vec, this.fwd_vec, this.up_vec);
        vec3.normalize(this.right_vec, this.right_vec);   // Not necessary in most situations. However, in theory,
                           // "up_vec" could be set in set_up_vec() such that it is not at a 90 degree angle to
                           // "fwd_vec". This could cause "rot_vert" to be calculated incorrectly in step 2 and
                           // ultimately cause "eye_pos" to be calculated incorrectly in step 6. This is especially
                           // noticable when "rads_x" and "rads_y" are large. The camera's eye position will deviate
                           // significantly from its original radius after just one call to traverse_arcball().

        // 2. Fill rotation quaternions with rotation angles.
        quat.setAxisAngle(this.rot_horiz, this.up_vec, -rads_x);
        quat.setAxisAngle(this.rot_vert, this.right_vec, -rads_y);

        // 3. Moving the mouse up and down rotates the forward and up vectors across the right camera axis.
        vec3.transformQuat(this.fwd_vec, this.fwd_vec, this.rot_vert);
        vec3.transformQuat(this.up_vec, this.up_vec, this.rot_vert);

        // 4. Moving the mouse left and right rotates the forward vector across the up camera axis.
        vec3.transformQuat(this.fwd_vec, this.fwd_vec, this.rot_horiz);

        // 5. Rebuild the right and up vector to prevent floating-point drift. Otherwise, vectors will slowly drift and
        // not stay at a 90 degree angle to each other due to accumulated floating-point rounding errors.
        vec3.cross(this.right_vec, this.fwd_vec, this.up_vec);
        vec3.cross(this.up_vec, this.right_vec, this.fwd_vec);
        vec3.normalize(this.up_vec, this.up_vec);   // This class stores the up vector in normalized form. It's
                                                    // normalized here for the next traverse_arcball() call.

        // 6. Calculate the new camera position based upon the new camera's forward vector.
        vec3.scale(this.fwd_vec, this.fwd_vec, this.eye_radius);    // Scale "fwd_vec" back to its original magnitude.
        vec3.subtract(this.eye_pos, this.targ_pos, this.fwd_vec);   // target position - forward vector
    }
};
...
class model
{
    ...
    // Orientation
    fwd_vec       = vec3.fromValues( 0.0,  0.0, 1.0);      // The forward vector of the object.
    ...
    // Matrix Outputs
    mdl_mtx       = mat4.create();                         // The model matrix of the object.

    //Intermediate Data
    right_vec     = vec3.fromValues(-1.0,  0.0, 0.0);      // An intermediate calculation for compute_mdl_mtx().
    ...
    // This function is not equivalent to targetTo(). targetTo() causes objects to face the wrong way (180 degree
    // rotation about the Y axis). This is because targetTo() orients models to face down the negative Z axis. In this
    // file, models are oriented to face the positive Z axis.
    compute_mdl_mtx()
    {
        vec3.subtract(this.fwd_vec, this.targ_pos, this.pos);   // target position - current position
        vec3.normalize(this.fwd_vec, this.fwd_vec);
        vec3.cross(this.right_vec, this.up_vec, this.fwd_vec);
        vec3.normalize(this.right_vec, this.right_vec);   // All three vectors are normalized by this point. The up
                                                          // vector i normalized in set_up_vec().
        mat4.identity(this.mdl_mtx);
        mat4.translate(this.mdl_mtx, this.mdl_mtx, this.pos);

        // Fill the rotation matrix in the model matrix with data from the right vector, up vector, and forward vector.
        this.mdl_mtx[0]  = this.right_vec[0]   * this.scale[0];   // Right vector with scaling
        this.mdl_mtx[1]  = this.right_vec[1]   * this.scale[0];
        this.mdl_mtx[2]  = this.right_vec[2]   * this.scale[0];
        this.mdl_mtx[4]  = this.up_vec[0]      * this.scale[1];   // Up vector with scaling
        this.mdl_mtx[5]  = this.up_vec[1]      * this.scale[1];
        this.mdl_mtx[6]  = this.up_vec[2]      * this.scale[1];
        this.mdl_mtx[8]  = this.fwd_vec[0]     * this.scale[2];   // Forward vecotr with scalin
        this.mdl_mtx[9]  = this.fwd_vec[1]     * this.scale[2];
        this.mdl_mtx[10] = this.fwd_vec[2]     * this.scale[2];
...
class particle_system extends model
{
    ...
    set_ref_particle(particle)
    {
        ...
        // Copy VBO data from the reference particle.
...
class static_cr_sim extends particle_system
{
    ...
    init_particles()
    {
        ...
        vec3.subtract(u, this.targ_pos, this.pos);   // target position - current position
...
class circle extends model
{
    constructor(gl, num_segs)
    {
        ...
        let ang = 0.0;
        positions.push(0.0, 0.0, 0.0);    // First add the center point, then go around the circle counter-clockwise.
        ...
        for(let i = 0; i < num_segs; i++)
        {
            ...
            st_coords.push(x + 0.5, -y + 0.5);    // Convert cartesian coordinates to ST coordinates. Y is inverted.
            ang += d_ang;
        }
...
class square extends model
{
    constructor(gl)
    {
        ...
        // if you are looking at it from the negative Z axis.
        let indices = [0, 1, 2, 0, 2, 3];    // Front Face - Faces the positive Z axis (facing towards the screen).
